...
def func0(xD_t_i):
	...
	def func2(xP_e):
		...
		v = abs((epsilon_d[0, 0] - (D_e**2 / xD_t_i**2)) / (D_e**2 / xD_t_i**2) * 100) # 目的関数
		return v
	...
	C_f[0, 0] = math.sqrt(2 * gamma[0, 0]**2 / (gamma[0, 0] - 1) * ((2 / (gamma[0, 0] + 1))**((gamma[0, 0] + 1) / (gamma[0, 0] - 1))) * (1 - (P_e[0, 0] / P_c[0, 0])**((gamma[0, 0] - 1) / gamma[0, 0]))) \
		+ ((P_e[0, 0] - P_o) / P_c[0, 0]) * ((D_e**2) / xD_t_i**2)
	F_t[0, 0] = ((1 + math.cos(math.radians(alpha))) / 2) * C_f[0, 0] * P_c[0, 0] * (math.pi * xD_t_i**2 / 4)
	I_t[0, 0] = 0
	...
	def func2(xP_e):
		...
		v = abs((epsilon_d[1, 0] - (D_e**2 / xD_t_i**2)) / (D_e**2 / xD_t_i**2) * 100) # 目的関数
		return v
	...
	C_f[1, 0] = math.sqrt(2 * gamma[1, 0]**2 / (gamma[1, 0] - 1) * ((2 / (gamma[1, 0] + 1))**((gamma[1, 0] + 1) / (gamma[1, 0] - 1))) * (1 - (P_e[1, 0] / P_c[1, 0])**((gamma[1, 0] - 1) / gamma[1, 0]))) \
		+ ((P_e[1, 0] - P_o) / P_c[1, 0]) * ((D_e**2) / (xD_t_i**2))
	F_t[1, 0] = ((1 + math.cos(math.radians(alpha))) / 2) * C_f[1, 0] * P_c[1, 0] * (math.pi * xD_t_i**2 / 4)
	...
	for k in range(2, len(t)):
		...
		def func2(xP_e):
			...
			v = abs((epsilon_d[k, 0] - (D_e**2 / xD_t_i**2)) / (D_e**2 / xD_t_i**2) * 100) # 目的関数
			return v
		...
		C_f[k, 0] = math.sqrt(2 * gamma[k, 0]**2 / (gamma[k, 0] - 1) * ((2 / (gamma[k, 0] + 1))**((gamma[k, 0] + 1) / (gamma[k, 0] - 1))) * (1 - (P_e[k, 0] / P_c[0, 0])**((gamma[k, 0] - 1) / gamma[k, 0]))) \
			+ ((P_e[k, 0] - P_o) / P_c[k, 0]) * ((D_e**2) / xD_t_i**2)
		F_t[k, 0] = ((1 + math.cos(math.radians(alpha))) / 2) * C_f[k, 0] * P_c[k, 0] * (math.pi * xD_t_i**2 / 4)
	...
	I_sp = I_total / ((m_ox_total + m_f_total) * 9.8) # 目的関数
	return -I_sp

result = minimize_scalar(func0, method='bounded', bounds=(0, D_e))
D_t_i = result.x
...
print(f'Initioal nozzle diameter = {D_t_i} (mm)')
